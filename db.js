...
export async function initDB() {
  await pool.query(`
    ...
    ALTER TABLE clients ADD COLUMN IF NOT EXISTS emergency_available BOOLEAN DEFAULT false;
    ALTER TABLE clients ADD COLUMN IF NOT EXISTS emergency_surcharge VARCHAR(50);
    ALTER TABLE clients ADD COLUMN IF NOT EXISTS booking_lead_time VARCHAR(50);
  `);
...
export async function updateClient(id, data) {
  const { rows } = await pool.query(
    `UPDATE clients SET
       ...
       zapier_webhook_url=$16, job_duration=$17, pricing=$18,
       emergency_available=$19, emergency_surcharge=$20, booking_lead_time=$21
    ...
    [
      ...
      data.zapier_webhook_url || null, data.job_duration || null,
      data.pricing || null, data.emergency_available ?? false,
      data.emergency_surcharge || null, data.booking_lead_time || null, id,
    ]
  );
  return rows[0];
}
